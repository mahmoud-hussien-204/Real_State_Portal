...
const schema = Yup.object().shape({
  country_id: Yup.number().min(1, 'country is required').required('country is required'),
  city_id: Yup.number().min(1, 'city is required').required('city is required'),
  area_id: Yup.number().min(1, 'area is required').required('area is required'),
...
const OffersFilterHeader = ({setOffers}: {setOffers: Dispatch<SetStateAction<IOffer[]>>}) => {
  ...
  const {data: areasData, isFetching: isFetchingAreas} = useQuery({
    ...
    queryFn: () => apiGetAreasOfCity(cityId), 
    enabled: cityId > 0
  });
  ...
  const {refetch, isFetching: isFetchingOffers, data: offersData} = useQuery({
    queryKey: ['offers'], 
  ...
  return (
    <form 
      ...
      className='relative z-[200] mx-auto mt-[-3rem] flex max-w-[90%] flex-wrap items-center justify-between gap-y-4 gap-x-2 rounded-lg bg-white px-5 py-5 shadow-md lg:max-w-[65rem] lg:flex-nowrap lg:rounded-full'
    >
      <SelectInput
      ...
      <SelectInput
        ...
        value={cityId > 0 ? cityId.toString() : undefined}
      ...
      <SelectInput
        ...
        value={areaId > 0 ? areaId.toString() : undefined}
        icon={isFetchingAreas ? <CgSpinner className="animate-spin" /> : undefined}
        disabled={cityId <= 0 || isFetchingAreas}
      />
      <SelectInput
      ...
      <Button 
        ...
        disabled={countryId <= 0 || cityId <= 0 || areaId <= 0 || categoryId <= 0 || isFetchingOffers}
